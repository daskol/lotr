...
from lotr import LoRALinear, map_module

RE_PARAM_LOW_RANK = re.compile(r'roberta\.encoder\.layer\.\d+\.'
                               r'attention\.self\.(value|query)\.factors.[01]')
...
opt_optim.add_argument('--lr', default=2e-5, type=float, help='learning rate')
...
def convert_model(model, rank: int):
    return map_module(
        model, partial(convert_low_rank, rank),
...
def instantiate_model(model_path='roberta-base', enable_lora=True,
                      num_labels=2,
                      rank=1) -> RobertaForSequenceClassification:
    model = RobertaForSequenceClassification \
        .from_pretrained(model_path, num_labels=num_labels) \
        .requires_grad_(False)
    if enable_lora:
        # logger.info('patch model to add correction term')
        model = convert_model(model, rank)
        model = mask_weights(model)
        model.classifier.requires_grad_(True)
        for layer in model.roberta.encoder.layer:
            layer.attention.self.query.factors.requires_grad_()
            layer.attention.self.value.factors.requires_grad_()
    return model
...
def instantiate_optimizer(model, lr, rank, batch_size, num_epoches,
    ...
    params = chain(params_cls, params_lr)
    ...
    opt = T.optim.AdamW(params, **opt_kwargs)
...
def make_trainer(task: str, batch_size=16, num_epoches=1, enable_lora=False,
    ...
    model = instantiate_model(model_path, enable_lora, num_labels, rank)
...
def train(task: str, batch_size=16, num_epoches=1, enable_lora=False, rank=1,
          lr=2e-5, log_dir: Optional[Path] = None,
          seed: Optional[int] = None):
    trainer = make_trainer(task, batch_size, num_epoches, enable_lora, rank,
                           lr, log_dir, seed)
...
def main(args: Namespace):
    logging.basicConfig(format='%(asctime)s %(levelname)s %(message)s',
                        level=logging.INFO)
    train(args.task, args.batch_size, args.num_epoches, args.enable_lora,
          args.rank, args.lr)
